...
//  void fill_trigInfo(const edm::TriggerResults& triggerResults, const edm::TriggerNames& trigNames);
  ...
  std::vector<int>   getPho175TrigCode_;
  std::vector<bool>  evtPassesPho175L1_;
  std::vector<bool> passFilter_;
...
void MonoNtupleDumper::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  iEvent.getByToken(trigEvent, handle_trigEvent);
...
MonoNtupleDumper::beginJob()
{
  ...
  m_tree->Branch("getPho175TrigCode", &getPho175TrigCode_);
  m_tree->Branch("evtPassesPho175L1" , &evtPassesPho175L1_);
  m_tree->Branch("passFilter" , &passFilter_);
