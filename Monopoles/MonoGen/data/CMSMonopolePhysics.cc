...
CMSMonopolePhysics::CMSMonopolePhysics(const HepPDT::ParticleDataTable * pdt,
  ...
  //cout << "Hello from CMSMonopolePhysics!" << endl;
  ...
  if (pdt) {
	...
	if (verbose > 0) std::cout << "CMSMonopolePhysics: Monopole[" << ii
				<< "] " << particleName << " Mass "
  ...
  if (verbose > 0) std::cout << "CMSMonopolePhysics has " << names.size()
...
void CMSMonopolePhysics::ConstructParticle() {
  ...
  for (unsigned int ii=0; ii<names.size(); ++ii) {
    if (!monopoles[ii]) {
      ...
      monopoles[ii] = mpl;
      if (verbose > 0) cout << "Create G4Monopole " << names[ii] 
...
void CMSMonopolePhysics::ConstructProcess() {
  ...
  if (verbose > 0)
    cout << "### CMSMonopolePhysics ConstructProcess()" << endl;
  for (unsigned int ii=0; ii<monopoles.size(); ++ii) {
    if (monopoles[ii]) {
      ...
      if (verbose > 1)
	cout << "### CMSMonopolePhysics instantiates for " << particleName 
      ...
      G4int idx = 1;
      if (verbose > 1)
	cout << "### Magnetic charge " << magn << " and electric charge " 
	       << mpl->GetPDGCharge() <<"\n   # of bins in dE/dx table = "
	       << nbin << " in the range " << emin << ":" << emax << endl;
